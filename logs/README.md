...
- **Named experiment trials** bypass this folder entirely. Launching with `ros2 launch qr_nav qr_nav.launch.py session_id:=t1_per_command` writes the CSV to `<repo>/results/t1_per_command_<TS>.csv` instead. The console log still lands under `logs/session_<TS>/launch_qr_nav_<TS>/` when the auto-logging hook is active, so you keep the cmd_meta.txt + console.log pair, but the dataset itself lives under `results/` ready for `analyze.ipynb`. See `phases/phase-2-experiments.md`.
